...
// withDefaults 是 Vue 编译期 macro，调用本身注册 props 默认值，无需赋值
withDefaults(
  defineProps<{
    size?: number;
    floating?: boolean;
...
<template>
  ...
  >
    <svg viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg" class="robot-svg">
      ...
      <rect
        x="22" y="22" width="76" height="58" rx="18" ry="18"
        fill="none" stroke="var(--robot-eye)" stroke-width="0.8" opacity="0.6"
      />

      <!-- 头部装饰线：赛博风侧边线条 -->
      <line x1="26" y1="30" x2="26" y2="72" stroke="var(--robot-antenna)" stroke-width="0.8" opacity="0.5" />
      ...
      <rect
        x="32" y="80" width="56" height="32" rx="12" ry="12"
        fill="none" stroke="var(--robot-eye)" stroke-width="0.8" opacity="0.5"
      />
      ...
      <line x1="76" y1="88" x2="84" y2="88" stroke="var(--robot-antenna)" stroke-width="0.8" opacity="0.6" />
    </svg>
  </div>
</template>
...
.robot-avatar {
  ...
  filter: drop-shadow(0 0 12px var(--robot-glow));
}
