...
// 对全新路径的 create+write 允许。故每次部署都新建 api/public_live_<ts>，
...
if (!SRC) {
  console.error('用法: node api/_deploy_live.mjs <构建目录>  （例如 builds/dist_u140557）');
  process.exit(1);
}
...
// 锚定到脚本所在目录（api/），无论从哪个 cwd 调用都能落到 getApiDir() 能扫描到的位置
const ts = Date.now();
const DST = path.resolve(scriptDir, 'public_live_' + ts);
