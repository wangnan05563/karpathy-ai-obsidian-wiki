...
export const useCompileStore = defineStore('compile', () => {
    ...
    const draftBatchItems = ref([]);
    const draftIsPublishing = ref(false);
    const draftProgressMessage = ref('');
    ...
    const currentStepStartedAt = ref(0);
    // §优化方案1：编译整体开始时间戳，用于展示总耗时
    const compileStartedAt = ref(0);
    ...
    const progressPercentage = computed(() => {
        if (!isCompiling.value && !isDone.value)
            return 0;
    ...
    function startDraftPublish(items) {
    ...
    function startSingleDraftPublish(item) {
    ...
    function appendDraftItemPage(index, page) {
        const item = draftBatchItems.value[index];
        if (!item)
            return;
    ...
    function finalizeDraftPublish() {
        draftIsPublishing.value = false;
        persistDraftPublishState();
    }
    // 主动清空草稿发布面板（用户点击关闭按钮）
    function clearDraftPublish() {
        if (draftAbortController) {
            draftAbortController.abort();
            draftAbortController = null;
        }
    ...
    function persistDraftPublishState() {
        try {
            const state = {
    ...
    function loadDraftPublishState() {
        try {
    ...
    // 清除草稿发布持久化状态
    function clearDraftPublishPersistedState() {
        try {
    ...
    function prepareBatchCompile(payload) {
        reset();
    ...
    function handleBatchPage(group, d) {
        group.timeline.push({
            step: d.step,
            status: d.status,
            message: d.message,
    ...
    function handleFileComplete(group) {
        if (group.status !== 'error') {
            group.status = 'done';
        }
    }
    // 批量模式事件分发：先处理整体事件，再按 fileIndex 路由到分组
    function handleBatchEvent(eventType, data) {
        const d = data;
