...
const QUERY_EVENT_HANDLERS = {
    ...
    ppt: (parsed, store) => {
        ...
        if (parsed.threadId)
            store.setThreadId(parsed.threadId);
...
export function parseSSEEvent(evt) {
...
export function dispatchQuerySSEEvent(eventType, data, store) {
...
// 批量处理 SSE 事件数组
export function processQuerySSEEvents(events, store) {
...
export async function consumeQuerySSE(response, store, signal) {
    if (!response.body)
...
// 非 JSON 数据跳过，不抛异常打断流
export async function consumeSSE(response, handler, signal) {
    if (!response.body)
        throw new Error('Response body is empty');
    const reader = response.body.getReader();
    const decoder = new TextDecoder();
