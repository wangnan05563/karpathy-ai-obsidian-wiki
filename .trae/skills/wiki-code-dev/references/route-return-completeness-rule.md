# Rule Catalog — 路由 return 完整性 (CODING-ROUTE-RETURN-COMPLETENESS)

通用后端编码规范：Fastify 路由处理函数中，**每个分支都必须 `return` / `reply`**，禁止任何执行路径"漏 return"后落到函数末尾。Fastify 在 handler 返回 `undefined` 时会隐式调用 `reply.send()`，若此前已 `res.send` / 已 `return reply.xxx` 则触发 `ERR_STREAM_WRITE_AFTER_END` 双发响应，或静默挂起使前端收到空响应/超时。本规则是后端审查条目 `wiki-backend-code-review` BR-089 的上位规范（扩展 BR-ESM-04 端点可达性）。
...
### CODING-ROUTE-RETURN-COMPLETENESS-1: 每个分支都必须 return/reply
...
### CODING-ROUTE-RETURN-COMPLETENESS-2: async handler 禁止隐式 undefined 落底
